...
## 物理層：
	...
	&emsp;&emsp 物理層是OSI模型中的最底層的模型，它定義了以比特作爲信號在信道上發送時相關的電氣，時序和
...
### 數據通信的理論基礎：
	...
	&esmp;&esmp 改變諸如電壓或電流等某種物理特性的方法可用來在電線上傳輸信息。若用一個以時間t爲自變量的
單值函數f(t)來表示電壓或電流的值，就可對信號的行爲進行建模，再用數學手段對其分析。


	&emsp;&emsp 傅里葉分析：  
		
	&esmp;&emsp （下面將'_'記爲級數的符號，將'..'記爲無窮，將'pai'記爲圓周率，符號'之後的值意
